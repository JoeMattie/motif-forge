...
import { bassNotes, crossPartScore, padNotes, progressionFor } from './harmony'
 ...
 * survivor gains a seeded drum part; with extraInstruments it gains the
 * chord scaffold — a seeded bass line (best of BASS_TAKES by inter-track
 * consonance) and a sustained pad following the batch progression.
 */
export function generateSymbolicBatch(
  ...
  const motifs = picked.map((ind, rank) => {
    ...
    // lead stays part 0 (melodicLine/bay/crossover depend on it), bass and pad
    // stay inside pitch 36–96 and add ≤4 sustained voices under a mono lead.
    const parts: Part[] = [{ name: 'lead', instrument: 'synth' }]
    const layers: Note[][] = [ind.notes]
    const tags: string[] = []

    if (brief.extraInstruments) {
      const harmonyCtx = {
        bars: ind.ctx.bars,
