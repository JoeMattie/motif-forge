...
- `src/api/` — Anthropic API client (`claude-sonnet-4-6`, no API key header in this environment), prompt builders (enumerate allowed pitch classes concretely), fence-stripping JSON extraction, and batch orchestration (10/call, `max_tokens` 8000; a `max_tokens` stop reason means truncated JSON — retry smaller, never parse it).
