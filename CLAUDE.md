...
Vite + React 19 + TypeScript strict. Runtime deps: `react`/`react-dom`, `@mantine/core`/`@mantine/hooks` (UI components), `tone` (synth playback), `smplr` (sampled instruments). The MIDI writer, WAV encoder, and IndexedDB wrapper remain hand-rolled by design; do not add libraries for those.

UI chrome (buttons, selects, inputs, checkboxes, badges) is Mantine, themed in `src/theme.ts` (dark scheme, `forge` accent palette, grays mapped onto the studio palette). Hover descriptions are Mantine `Tooltip`s (arrow/delay/multiline come from theme defaults — don't use `title` attributes); layout rows/columns are `Group`/`Stack`, dim helper text is `<Text c="dimmed">`. The `:root` font-size of 13px deliberately shrinks Mantine's rem-based component dimensions to a compact fit; text sizes are pinned in px in the theme, so keep both in sync. Mantine's PostCSS preset is configured in `postcss.config.cjs`. Layout, motif cards, and the piano roll stay hand-rolled CSS in `src/styles.css` against the CSS variables in `:root` — don't convert those to Mantine.
